...
contract MyUSDEngine is Ownable {
    ...
    function _getCurrentExchangeRate() internal view returns (uint256) {
        ...
        uint256 interestAccrued = (debtExchangeRate * borrowRate * timeElapsed) / (10000 * SECONDS_PER_YEAR);
        return debtExchangeRate + interestAccrued;
    }
    ...
    function _getDebtValueFromShares(uint256 debtShares) internal view returns (uint256) {
        if (debtShares == 0) return 0;
        return (debtShares * _getCurrentExchangeRate()) / PRECISION;
    }
    ...
    function _validatePosition(address user) internal view {
        ...
        if (positionRatio == type(uint256).max) return;
        if ((positionRatio * 100) < COLLATERAL_RATIO * PRECISION) revert Engine__UnsafePositionRatio();
    }
    ...
    function mintMyUSD(uint256 mintAmount) public {
        ...
        uint256 newDebtShares = s_userDebtShares[msg.sender] + shares;
        uint256 newDebtValue = _getDebtValueFromShares(newDebtShares);
        uint256 collateralValue = calculateCollateralValue(msg.sender);
        if (newDebtValue != 0) {
            uint256 newPositionRatio = (collateralValue * PRECISION) / newDebtValue;
            if ((newPositionRatio * 100) < COLLATERAL_RATIO * PRECISION) {
